namespace :db do
  ...
  task :version do
    version = if SEQUEL_DB.tables.include?(:schema_info)
      SEQUEL_DB[:schema_info].first[:version]
    end || 0
  ...
  task :migrate do
    Sequel::Migrator.run(SEQUEL_DB, "db/migrate")
  ...
  task :rollback, :target do |t, args|
    ...
    Sequel::Migrator.run(SEQUEL_DB, 'db/migrate', target: args[:target].to_i)
    Rake::Task['db:version'].execute
  end

  desc "Perform migration reset (full rollback and migration)"
  task :reset do
    Sequel::Migrator.run(SEQUEL_DB, "db/migrate", :target => 0)
    Sequel::Migrator.run(SEQUEL_DB, "db/migrate")
    Rake::Task['db:version'].execute
  end
  ...
  task :migration, :name do |_, args|
    ...
    filename = File.join('db/migrate', "#{timestamp}_#{args[:name]}.rb")
